package livemarket.articleread.repository;

import livemarket.common.dataserializer.DataSerializer;
...
public class ArticleQueryModelRepository {
    ...
    public void create(ArticleQueryModel articleQueryModel, Duration ttl) {
        redisTemplate.opsForValue()
                .set(generateKey(articleQueryModel), Objects.requireNonNull(DataSerializer.serialize(articleQueryModel)), ttl);
    }

    public void update(ArticleQueryModel articleQueryModel) {
        redisTemplate.opsForValue().setIfPresent(generateKey(articleQueryModel), Objects.requireNonNull(DataSerializer.serialize(articleQueryModel)));
    }
