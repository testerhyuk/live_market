package livemarket.articleread.data;
...
public class DataInitializer {
    ...
    Long createArticle() {
        return articleServiceClient.post()
                .uri("/v1/articles")
                .body(new ArticleCreateRequest("title", "content", 1L, 1L, List.of("image1.jpg")))
                .retrieve()
    ...
    void like(Long articleId, long likeCount) {
        while(likeCount-- > 0) {
            likeServiceClient.post()
                    .uri("/v1/article-likes/articles/{articleId}/users/{userId}", articleId, likeCount)
                    .retrieve()
                    .body(String.class);
        }
    }
